...
// ============================================================
// CLASS: GameDataManager — 雲端登入與註冊連接
// ============================================================
  ...
  static async loginOrRegisterRemote(trainerName, password) {
    try {
      ...
      if (!res.ok || !result.success) {
        throw new Error(result.message || '連線伺服器失敗');
      }
      return {
...
// ============================================================
// CLASS: PlayerSessionManager — 訓練員進度管理
// ============================================================
class PlayerSessionManager {
  ...
  async doCloudAuth(name, password) {
    ...
    if (authResult.message.includes('建立新帳號')) {
      alert(`🎉 歡迎新訓練員 ${name}！已為您在雲端建立全新帳戶！`);
    }
    return true;
  }
...
// ============================================================
// CLASS: BattleEngine — 二位數心算生成器 (5秒)
// ============================================================
class BattleEngine {
  constructor() {
    this.duration = 5000; 
...
class UIManager {
  ...
  nextQuestion() {
    this.qCount++; this.currentQuestion = this.battle.generateQuestion();
    ...
    this.currentQuestion.options.forEach((val, i) => {
      if (btns[i]) { btns[i].textContent = val; btns[i].className = 'btn-ans'; }
    });

    this.battle.startTimer(
      (sec) => {
        document.getElementById('battle-time').textContent = sec.toFixed(1);
        document.getElementById('timer-fill').style.width = (sec / 5.0 * 100) + '%';
      },
      () => { this.audio.play('wrong'); this.endBattle(); }
    );
  }
  ...
  handleAnswer(idx) {
    ...
    if (isCorrect) {
      ...
      document.getElementById('battle-score').textContent = this.score;
      setTimeout(() => this.nextQuestion(), 400);
    } else { this.audio.play('wrong'); setTimeout(() => this.endBattle(), 600); }
  }

  async endBattle() {
    this.battle.stopTimer(); this.session.recordBattle(this.cCount, this.qCount);
    ...
    if (this.cCount >= 3) {
      ...
      if (uncaught.length > 0) {
        ...
        document.getElementById('capture-text').innerHTML = `答對 ${this.cCount} 題！能量充能完畢！<br><span style="color:var(--yellow);font-size:24px;">成功捕捉 ${target.name}！</span>`;
        document.getElementById('capture-sprite').src = `${this.spriteBase}${target.id}.png`;
        
        const overlay = document.getElementById('capture-overlay');
        overlay.classList.add('active'); this.audio.play('capture');

        setTimeout(() => { overlay.classList.remove('active'); this.showScreen('result-screen'); }, 3200);
        await this.session.save(); return;
      }
    }
    await this.session.save(); this.showScreen('result-screen');
  }
  ...
  showPokedex() {
    ...
    ALL_POKEMONS.forEach(p => {
      ...
      card.innerHTML = `
        ...
        <img class="dex-img" src="${this.spriteBase}${p.id}.png">
  ...
  _bindAll() {
    ...
    if (loginBtn) {
      loginBtn.addEventListener('click', async () => {
        ...
        if (!password) { alert('請輸入密碼！\n(新玩家將以此密碼自動在雲端建立帳戶)'); return; }

        // 💡 只有玩家按了按鈕，才把原來的 loading-overlay 打開，顯示「連線雲端中...」
...
// 核心修正：頁面載入完成時，強制立刻隱藏 loading-overlay 遮罩，直接秒出登入畫面
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 💡 關鍵：一進網頁，立刻不留情面地隱藏 loading-overlay！
  const overlay = document.getElementById('loading-overlay') || document.querySelector('.loading-overlay');
  if (overlay) overlay.style.display = 'none';
