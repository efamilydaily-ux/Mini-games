...
class UIManager {
  ...
  _playArrowAnimation(btnEl, isCorrect, berry) {
    ...
    const cx = rect.left + 40; // near berry icon
    ...
    if (isCorrect) {
      ...
      berryEl.textContent = berry.emoji;
      berryEl.style.left = `${cx - 10}px`;
      berryEl.style.top  = `${cy - 10}px`;
      layer.appendChild(berryEl);
      berryEl.addEventListener('animationend', () => berryEl.remove());
    }
  }
